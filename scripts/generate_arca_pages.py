...
root = Path("d:/Kerja/InJourney")
app_arca = root / "app" / "arca"
text = (app_arca / "arcaData.ts").read_text("utf-8")
entries = re.findall(r"id:\s*\"([^\"]+)\"[\s\S]*?number:\s*\"([^\"]+)\"[\s\S]*?title:\s*\"([^\"]+)\"[\s\S]*?bgImage:\s*\"([^\"]+)\"", text)

if not entries:
    raise SystemExit("No entries found in arcaData.ts")
...
for artifact_id, number, title, bgImage in entries:
    folder = app_arca / artifact_id
    folder.mkdir(parents=True, exist_ok=True)
    component_name = ''.join(part.capitalize() for part in title.replace('-', ' ').split())
    file_path = folder / 'page.tsx'
    file_path.write_text(
        page_template
        .replace('__COMPONENT_NAME__', component_name)
        .replace('__TITLE__', title)
        .replace('__BGIMAGE__', bgImage),
        'utf-8'
    )

legacy = app_arca / '[artifact]'
if legacy.exists() and legacy.is_dir():
    for child in legacy.iterdir():
        child.unlink()
    legacy.rmdir()

print(f"Created {len(entries)} artifact pages.")
